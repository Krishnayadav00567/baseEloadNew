...
export default function CreateMATcode(props) {
    const { URLS, ajaxUtil, loadingFunction, setNotification, close, onSuccess } = props
    ...
    const [values, fields, handleChange, { validateValues }] = useFieldItem(FormElements, {}, {onValueChange});
    ...
    const onCreateClick = () => {
        ...
        const request = {
            denominationId: values.denomination.value ? values.denomination.value : '',
            matCode: values.matCode,
            pocketId: values.pocketTypeId.value,
            // pocketName: values.pocketTypeId.label
        }

        ajaxUtil.sendRequest(URLS.CREATE_URL, request, (response, hasError) => {
            if (!hasError) {
                onSuccess()
                close()
            }
        }, loadingFunction, { method: 'POST' });
    }
    return (
        <ModalBody>
