...
export default function CreateRoleNew(props) {
  const [isSuccess, setIsSuccess] = useState(false);
  const [modules, setModules] = useState([]);
  const [modal, setModal] = useState(0)
  const [moduleDtls, setModuleDtls] = useState({});
  const [subModules, setSubModules] = useState([]);
  const [values, fields, handleChange, { validateValues, reset }] = useFieldItem(FIELDS, {}, {});
  const [index,setIndex]=useState("");
  useEffect(() => {
    props.setHeader("Roles");
    props.ajaxUtil.sendRequest(`${props.url_Roles.GET_MODULES}`, {}, (response, hasError) => {
      if (!hasError && response) {
        setModules(response.response.length>0?response.response.map(mod=>({...mod,moduleId:mod.id,subModuleList:[]})):[])
      } else {
        setModules([])
      }
  ...
  const onSaveClick = () => {
    ...
    if (hasError) {
      ...
      if (moduleError){//atleast one feature added
        ...
        props.ajaxUtil.sendRequest(`${props.url_Roles.CREATE_URL}`, req, (response, hasError) => {
          if (!hasError) {
              setIsSuccess(true);
          }
      }, props.loadingFunction, { method: 'POST', isShowSuccess: true});
    }else {
      props.setNotification({
        hasError: true,
  ...
  const formatSubModule = (subModule,parentId) => {
    return subModule.map(subMod => ({ ...subMod,parentId,featureList: subMod.featureList.map((ft) => ({ ...ft, devices: ft.devices && ft.devices.value || '' })) }))
  }
  if (isSuccess) {
              ...
              <tbody>
                {
                  modules.length > 0 ? modules.map((mod, key) => (<tr key={key}>
                    ...
                    <td><i className="cursor" style={{ cursor: "pointer" }} class="fa fa-pencil" aria-hidden="true"
